...
function slide (s) {
    ...
    if (img === 8) {
        grammeydys.style.display = 'block';
        grammAarloo();
    }
    else {
        grammeydys.textContent = '';
        grammeydys.style.display = 'none';
    }
}

function picOpacity () {
    if (img > 0) gsap.to("#pic" + (img-1), {opacity: 0});
    if (img < panels.length-1) gsap.to("#pic" + (img+1), {opacity: 0});
